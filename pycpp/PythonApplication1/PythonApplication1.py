...
def pos():
    ...
    p_fitness = fitness_func(X)  #计算粒子的的适应度值，也就是目标函数值，X 的维度是 size * 2
    print(f'初始化粒子自适应度:{p_fitness}')
    g_fitness = p_fitness.min()  #找到所有随机点里的最小值
    print(f'保存最小自适应度:{g_fitness}')
    fitness_val_list.append(g_fitness)  #压入数组

    # 初始化的个体最优位置和种群最优位置
    pbest = X
    gbest = X[p_fitness.argmin()]   #找到随机点中自适应度最小的在点坐标
    print(f'保存最小自适应度粒子的坐标:{gbest}')
    # 迭代计算
    for i in range(1, iter_num):
        V = velocity_update(V, X, pbest, gbest, c1, c2, w, max_val)
        X = position_update(X, V)
        if i<=3:
            print(f'第{i+1}次迭代后的速度{V}')
            print(f'第{i+1}次迭代后的位置{X}')
        p_fitness2 = fitness_func(X)  #计算运动后粒子的的适应度值，也就是目标函数值，X 的维度是 size * 2
        g_fitness2 = p_fitness2.min()   #找到运动后所有随机点里的最小值
        ...
        for j in range(size):
            ...
            if g_fitness > g_fitness2:
                gbest = X[p_fitness2.argmin()]#找到随机点中自适应度最小的在点坐标
    ...
    plt.scatter(gbest[0], gbest[1], color='r')
    plt.title('最终结果')
    plt.show()
    # 绘图
