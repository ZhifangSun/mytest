...
# 使用heapq实现优先队列
#定义一个可比较对象
class CompareAble(Task):

    def __init__(self, task=Task()):
        self.task=task

    def __lt__(self, other):
        if self.task.rank_sum_value <= other.task.rank_sum_value:
            return False
        else:
            return True
...
class CpopScheduler(Scheduler):
    ...
    def schedule(self, app):
        ...
        ComputingSystem.reset()  # 重置计算系统. !!!VERY IMPORTANT!!!

        processors = ComputingSystem.processors  # 处理器集
        tasks=app.tasks
        pd=PriorityQueue()
        pd.put(CompareAble(tasks[0]))
        t=[]
        while not pd.empty():
            q=pd.get_nowait()
            t.append(q.task)
            for j in q.task.successors:
                temp_task=j
                temp_task.in_degree-=1
                if temp_task.in_degree==0:
                    pd.put(CompareAble(temp_task))

        processor = None  # 全局处理器

        for task in t:  # 遍历排序任务集

            earliest_start_time = 0.0  # 初始化全局最早启动时间为0
            earliest_finish_time = float("inf")  # 初始化全局最早结束时间为无穷大
            if task.is_critical:
                for i in processors:
                    if i.is_critical_path:
                        earliest_start_time_of_this_processor=SchedulerUtils.calculate_earliest_start_time(task, i)
                        earliest_finish_time_of_this_processor=SchedulerUtils.calculate_earliest_finish_time(task, i)
                        break
                ...
                self.original_scheduling_list.list[task] = assignment  # 将任务与对应运行时环境置于原始调度列表
                ...
                for p in processors:  # 遍历处理器集
                    ...
                    earliest_finish_time_of_this_processor = SchedulerUtils.calculate_earliest_finish_time(task, p)  # 当前遍历处理器上最早可用结束时间
        ...
        self.original_scheduling_list.makespan = calculate_makespan(self.original_scheduling_list)  # 计算原始调度列表的完工时间

        print("The scheduler = %s, list_name = %s, makespan = %.2f" % (self.scheduler_name,
                                                                       self.original_scheduling_list.list_name,
                                                                       self.original_scheduling_list.makespan))

        if SHOW_ORIGINAL_SCHEDULING_LIST:  # 如果打印原始调度列表, 则:
            print_scheduling_list(self.original_scheduling_list)  # 打印原始调度列表
